...
let createPersonInvolvedForm = async(req, res) =>{
  ...
  try {
    ...
    await db.query(`INSERT INTO tcdh_persona_temp(
      ...
      fec_ing_reg, cod_usu_mod, usu_mod_reg, fec_mod_reg, id_caso_temp)
...
let updatePersonInvolvedForm = async (req, res) => {
  ...
  try {
    ...
    tipo_rel_caso=$30 
    WHERE id_persona_temp = $31 RETURNING*`, [per_den_es_victima, per_principal, nombre, apellido, id_cat_cal_actua, nom_completo, sexo, ide_genero, 
      lee, escribe, id_cat_doc_persona, otro_doc_identidad, num_documento, id_niv_academico, id_pais_nacimiento, id_pais_ins_rep, 
      fec_nacimiento_nueva, edad_aprox, id_cat_pro_oficio, zona_domicilio, id_departamento, id_municipio, domicilio, discapacidad, 
      id_cat_tip_discapacidad, med_rec_notificacion, cod_user, user_name, registration_date, tipo_rel_caso, id_persona_temp], async(err, results) => {
      if (err) {
        console.log(err.message);
        return res.status(500).json(errorResponse.toJson());
      }else{
